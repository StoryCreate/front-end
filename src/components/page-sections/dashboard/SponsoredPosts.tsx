import React from 'react';
import { Eye, Heart, Share2, Star, Plus } from 'lucide-react';

const SponsoredPosts = () => {
  return (
    <div>
      <h2 className="text-2xl font-semibold mb-4 text-left">Sponsored Posts</h2>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-4">
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <img
            className="w-full h-52 object-cover"
            src="https://c.animaapp.com/qFkmpUOx/img/rectangle-45-9@2x.png"
            alt="PERSEPOLIS"
          />
          <div className="p-4">
            <h3 className="text-lg font-medium text-left">PERSEPOLIS</h3>
            <p className="text-sm text-gray-500 text-left">By Marjane Satrapi</p>
            <div className='flex flex-row justify-between'>
              <div className="flex items-center mt-2 text-gray-500">
                <Eye className="w-4 h-4 mr-1" /> <span className="text-sm">800K</span>
              </div>
              <div className="flex items-center mt-4 space-x-4">
                <Heart className="w-5 h-5 text-black hover:text-primary cursor-pointer" />
                <Share2 className="w-5 h-5 text-gray-500 hover:text-primary cursor-pointer" />
                <button className="p-1 bg-black text-white rounded-full hover:bg-primary cursor-pointer">
                  <Plus className="w-4 h-4" />
                </button>
              </div>
            </div>
            <div className="flex items-center mt-2">
              {[...Array(5)].map((_, i) => (
                <Star key={i} className="w-4 h-4 text-primary" />
              ))}
              <span className="ml-2 text-sm text-gray-500">5/5 ratings</span>
            </div>
          </div>
        </div>
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <img
            className="w-full h-52 object-cover"
            src="https://c.animaapp.com/qFkmpUOx/img/rectangle-45-19@2x.png"
            alt="THE GIRL ON THE TRAIN"
          />
          <div className="p-4">
            <h3 className="text-lg font-medium text-left">THE GIRL ON THE TRAIN</h3>
            <p className="text-sm text-gray-500 text-left">By Paula Hawkins</p>
            <div className='flex flex-row justify-between'>
              <div className="flex items-center mt-2 text-gray-500">
                <Eye className="w-4 h-4 mr-1" /> <span className="text-sm">800K</span>
              </div>
              <div className="flex items-center mt-4 space-x-4">
                <Heart className="w-5 h-5 text-black hover:text-primary cursor-pointer" />
                <Share2 className="w-5 h-5 text-gray-500 hover:text-primary cursor-pointer" />
                <button className="p-1 bg-black text-white rounded-full hover:bg-primary cursor-pointer">
                  <Plus className="w-4 h-4" />
                </button>
              </div>
            </div>
            <div className="flex items-center mt-2">
              {[...Array(5)].map((_, i) => (
                <Star key={i} className="w-4 h-4 text-primary" />
              ))}
              <span className="ml-2 text-sm text-gray-500">5/5 ratings</span>
            </div>
          </div>
        </div>
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <img
            className="w-full h-52 object-cover"
            src="https://c.animaapp.com/qFkmpUOx/img/rectangle-45-7@2x.png"
            alt="THE SANDMAN"
          />
          <div className="p-4">
            <h3 className="text-lg font-medium text-left">THE SANDMAN</h3>
            <p className="text-sm text-gray-500 text-left">By Neil Gaiman</p>
            <div className='flex flex-row justify-between'>
              <div className="flex items-center mt-2 text-gray-500">
                <Eye className="w-4 h-4 mr-1" /> <span className="text-sm">35K</span>
              </div>
              <div className="flex items-center mt-4 space-x-4">
                <Heart className="w-5 h-5 text-black hover:text-primary cursor-pointer" />
                <Share2 className="w-5 h-5 text-gray-500 hover:text-primary cursor-pointer" />
                <button className="p-1 bg-black text-white rounded-full hover:bg-primary cursor-pointer">
                  <Plus className="w-4 h-4" />
                </button>
              </div>
            </div>
            <div className="flex items-center mt-2">
              {[...Array(5)].map((_, i) => (
                <Star key={i} className="w-4 h-4 text-primary" />
              ))}
              <span className="ml-2 text-sm text-gray-500">5/5 ratings</span>
            </div>
          </div>
        </div>
      </div>
    </div>
  )
}

export default SponsoredPosts